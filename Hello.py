...
st.subheader('1. Performance of Customer Orders Based on Their Order Status')

# Plot jumlah status order
fig1, ax = plt.pyplot.subplots(figsize=(10, 6))
sns.barplot(x=order_cust_status.index, y=order_cust_status.values, ax=ax)

# Menambahkan judul dan label sumbu
ax.set_title('Status Order Customer')
ax.set_xlabel('Status')
ax.set_ylabel('Number of the Status')

# Menampilkan nilai counts di dalam diagram batang
for i, value in enumerate(order_cust_status.values):
...
st.subheader('2. Number of Purchases per Year with Status "Delivered"')
...
st.subheader('3. Number of Purchases with Status "Delivered" per Month in 2017')
...
##4.
st.subheader('4. Top 10 Cities with the Most Sellers?')
...
for i, value in enumerate(top_10_cities['seller_id']):
    plt.pyplot.text(i, value + 5, str(value), ha='center', va='bottom')

# Menambahkan judul dan label sumbu
plt.pyplot.title('Top 10 Cities with Most Sellers')
plt.pyplot.xlabel('City')
plt.pyplot.ylabel('Number of Sellers')
...
##5.
st.subheader('5. Customer Demographic Based on the State')
# Plot jumlah status order
...
plt.pyplot.title("Number of Customers by States", loc="center", fontsize=15)
plt.pyplot.ylabel(None)
plt.pyplot.xlabel(None)
plt.pyplot.tick_params(axis='y', labelsize=12)
